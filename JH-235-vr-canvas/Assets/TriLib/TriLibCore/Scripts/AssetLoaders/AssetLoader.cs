...
namespace TriLibCore
{
    ...
    public static class AssetLoader
    {
        ...
        private static void LoadModelInternal(AssetLoaderContext assetLoaderContext)
        {
            ...
            {
                ThreadUtils.RequestNewThreadFor(assetLoaderContext, ref assetLoaderContext.CancellationToken, LoadModel,
                    ProcessRootModel, HandleError, assetLoaderContext.Options.Timeout, threadName, !assetLoaderContext.HaltTasks,
                    assetLoaderContext.OnPreLoad);
            }
        }
                ...
                if (hipsTransform != null)
                {
                    ...
                    for (var i = extraTransforms.Count - 1; i >= 0; i--)
                    {
                        ...
                        if (!skeletonBones.ContainsKey(extraTransform))
                        {
                            ...
                            //if (toBottom < 0f)
                            //{
                            //    var extraTransformPosition = extraTransform.position;
                            //    extraTransformPosition.y -= toBottom;
                            //    extraTransform.position = extraTransformPosition;
                            //}
                            //if (toCenter.sqrMagnitude > 0.01f)
                            //{
                            //    var extraTransformPosition = extraTransform.position;
                            //    extraTransformPosition += toCenter;
                            //    extraTransform.position = extraTransformPosition;
                            //}
        ...
        private static void ParseGeometry(AssetLoaderContext assetLoaderContext, GameObject meshGameObject, IRootModel rootModel, IModel meshModel)
        {
            ...
            {
#if UNITY_2020_2_OR_NEWER && TRILIB_LOAD_MESHES_ON_PARALLEL_THREAD
                geometryGroup.FinalMeshSetup(assetLoaderContext, meshModel.BindPoses);
#else
                geometryGroup.GenerateMesh(assetLoaderContext, assetLoaderContext.Options.AnimationType == AnimationType.None ? null : meshModel.BindPoses);
#endif
        ...
        private static void LoadModel(AssetLoaderContext assetLoaderContext)
        {
            ...
            ProcessGeometries(assetLoaderContext);
#else
            SetupModelLoading(assetLoaderContext);
#endif
        }
...
#if UNITY_2020_2_OR_NEWER && TRILIB_LOAD_MESHES_ON_PARALLEL_THREAD
        /// <summary>Creates Unity Meshes.</summary>
        /// <param name="assetLoaderContext">The Asset Loader Context reference. Asset Loader Context contains the Model loading data.</param>
        private static void ProcessGeometries(AssetLoaderContext assetLoaderContext)
        {
            var dataArrayCreationContext = new DataArrayCreationContext { Context = assetLoaderContext };
            if (assetLoaderContext.RootModel.GeometryGroup != null)
            {
                dataArrayCreationContext.Models.Add(assetLoaderContext.RootModel);
            }
            if (assetLoaderContext.RootModel.AllModels != null)
            {
                for (var i = 0; i < assetLoaderContext.RootModel.AllModels.Count; i++)
                {
                    var model = assetLoaderContext.RootModel.AllModels[i];
                    if (model.GeometryGroup != null)
                    {
                        dataArrayCreationContext.Models.Add(model);
                    }
                }
            }
            if (dataArrayCreationContext.Models.Count > 0)
            {
                Dispatcher.InvokeAsyncAndWait(CreateDataArray, dataArrayCreationContext);
                for (var i = 0; i < dataArrayCreationContext.Models.Count; i++)
                {
                    var model = dataArrayCreationContext.Models[i];
                ...
                Dispatcher.InvokeAsyncAndWait(ApplyDataArray, dataArrayCreationContext);
            }
        }

        private static void ApplyDataArray(DataArrayCreationContext dataArrayCreationContext)
        {
            dataArrayCreationContext.Meshes = new Mesh[dataArrayCreationContext.Models.Count];
            for (var i = 0; i < dataArrayCreationContext.Models.Count; i++)
            {
                var model = dataArrayCreationContext.Models[i];
                model.GeometryGroup.Mesh = new Mesh {name = model.GeometryGroup.Name};
                dataArrayCreationContext.Meshes[i] = model.GeometryGroup.Mesh;
            }
            Mesh.ApplyAndDisposeWritableMeshData(dataArrayCreationContext.DataArray, dataArrayCreationContext.Meshes);
        }

        private static void CreateDataArray(DataArrayCreationContext dataArrayCreationContext)
        {
            dataArrayCreationContext.DataArray = Mesh.AllocateWritableMeshData(dataArrayCreationContext.Models.Count);
            dataArrayCreationContext.Completed = true;
        }
#endif
        ...
        private static void ProcessMaterials(AssetLoaderContext assetLoaderContext)
        {
            if (assetLoaderContext.RootModel?.AllMaterials != null && assetLoaderContext.RootModel.AllMaterials.Count > 0)
            {
                if (assetLoaderContext.Options.MaterialMappers != null)
                {
                    ThreadUtils.RequestNewThreadFor(assetLoaderContext, ref assetLoaderContext.CancellationToken, ProcessMaterialRenderers, null, HandleError, assetLoaderContext.Options.Timeout, null, !assetLoaderContext.HaltTasks);
                }
