...
namespace Snobal.ConstraintUtilitiesUnity_0_0
{
    public static class ConstraintUtilities
    {
        ...
        public static PositionConstraint ApplyPositionConstraint(Transform _parent, Transform _child, float _weight = 1f)
        {
            PositionConstraint constraint = UpdateConstraint<PositionConstraint>(_parent, _child, _weight);

            Vector3 positionOffset = _parent.InverseTransformPoint(_child.position);

            constraint.translationOffset = positionOffset;            
        ...
        public static PositionConstraint ApplyPositionConstraint(Transform _parent, Transform _child, Vector3 localOffsetPos, float _weight = 1f)
        {
            PositionConstraint constraint = UpdateConstraint<PositionConstraint>(_parent, _child, _weight);

            Vector3 positionOffset = _parent.InverseTransformPoint(_child.position);

            constraint.translationOffset = localOffsetPos;
